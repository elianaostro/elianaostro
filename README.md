## Eliana Ostrovsky

**AI Engineering Student (4th year) | Top 10% of cohort (GPA 8.74)**

AI Engineer at **Bouldertech.ia** | Junior Data Analyst at **Alephee**

Reinforcement Learning for Physical Models & Robotics | AI Safety

---

### Professional & Academic Experience

**AI Engineer** &mdash; Bouldertech.ia
Developing and implementing AI solutions.

**Junior Data Analyst** &mdash; Alephee
Data processing and automation.

**Teaching Assistant** &mdash; Physics 1 and Mathematical Analysis 3

---

### Technical Expertise

**Languages & Package Management**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=ffdd54)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![uv](https://img.shields.io/badge/uv-DE5FE9?style=flat-square&logo=uv&logoColor=white)

**Robotics & Simulation**

![ROS2](https://img.shields.io/badge/ROS2-22314E?style=flat-square&logo=ros&logoColor=white)
![Unity](https://img.shields.io/badge/Unity-000000?style=flat-square&logo=unity&logoColor=white)
![ESP32](https://img.shields.io/badge/ESP32-E7352C?style=flat-square&logo=espressif&logoColor=white)

**Infrastructure**

![Linux Mint](https://img.shields.io/badge/Linux_Mint-87CF3E?style=flat-square&logo=linuxmint&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)

**Orchestration**

![n8n](https://img.shields.io/badge/n8n-EA4B71?style=flat-square&logo=n8n&logoColor=white)
![Microsoft 365](https://img.shields.io/badge/Microsoft_365-D83B01?style=flat-square&logo=microsoft&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)
![SharePoint](https://img.shields.io/badge/SharePoint-0078D4?style=flat-square&logo=microsoftsharepoint&logoColor=white)

---

### Technical Projects

**Autonomous Driving Simulation**
Implemented Proximal Policy Optimization (PPO) within Unity for autonomous vehicle navigation.
`Unity ML-Agents` `PyTorch` `PPO`

**Computer Vision Lane Detection**
Developed a teacher-student model architecture adapted for Argentine road conditions.
`PyTorch` `Computer Vision` `Knowledge Distillation`

**Physical Autonomous Airplane**
Engineered a ROS2-based flight control system using ESP32.
`ROS2` `ESP32` `Python`

**Enterprise Automation**
Architected internal agents using n8n for cross-platform Microsoft 365 integration.
`n8n` `Azure` `Microsoft 365` `SharePoint`

---

[![Email](https://img.shields.io/badge/elianaostro@gmail.com-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:elianaostro@gmail.com)
